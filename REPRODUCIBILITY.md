...
> **How memory is measured (both columns, both implementations).** The reported footprint is the **`MEM analytic bytes`**: a closed-form sum of every device allocation (engine = grammar rules + the two $w^{*}$ frontier buffers + I/O vectors; cuSPARSE = row-pointer + $8\,{\cdot}\,\text{nnz}$ CSR + vectors + cuSPARSE's own declared workspace). It is deterministic and architecture-independent. `MEM peak bytes` is the runtime high-water (`cudaMemGetInfo` delta from a baseline taken just before allocation, read after a warm-up) and is only a completeness cross-check — it tracks the analytic value, confirming no large allocation is unaccounted for, but varies run to run (allocator/page granularity) so it is not the reported number. Both implementations use the *same* allocator (`cudaMallocManaged`) and the *same* measure sequence, so the comparison is apples-to-apples; memory is reported only for the two GPU implementations (the CPU/GraphBLAS/cuGraph baselines are time/correctness references).
