...
for b in wd_sports_team wd_cast_member wd_citizenship wd_occupation wd_subclass_of; do
  echo "##### $b #####"
  timeout 300 ./cgvenv/bin/python cugraph_bench.py $Z/${b}.sparse ${DIM[$b]} bool 20 2>&1 | grep -E "cuGraph|Error|error" | tail -3
  timeout 300 ./cgvenv/bin/python cugraph_bench.py $Z/${b}.sparse ${DIM[$b]} tropical 20 2>&1 | grep -E "cuGraph|Error|error" | tail -3
done
